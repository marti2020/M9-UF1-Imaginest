<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
    {
        ...
        {
            $valid=0;
            $con=$db->prepare('SELECT mail from users where mail=:email or username=:email');
            $con->bindParam(':email',$_GET['mail']);
            if($con->execute())
            {
                $valid=$con->fetch(PDO::FETCH_ASSOC);
                if($_GET['code']!=$valid['resetPassCode'] or $_GET['mail']!=$valid['mail'] or !isset($_GET['code']) or !isset($_GET['mail']))
                {
        ...
        if(isset($_POST['pass1']) and isset($_POST['pass2']))
        {
            if($_POST['pass1']==$_POST['pass2'])
            {
                ...
                $correu=$_GET['mail'];
                $sql = "UPDATE users set passHash='$pass' where mail='$correu'";
                $smt = $db->prepare($sql);
                $smt->execute();
                header('Location: index.php');
            }
        }
        else if(!empty($_POST['gpass']))
        {
            $valid=0;
            $con=$db->prepare('SELECT active from users where mail=:email or username=:email');
            $con->bindParam(':email',$_POST['gpass']);
            $con->execute();
            $valid=$con->fetch(PDO::FETCH_ASSOC);
            
            if(empty($valid) or $valid['active']!=1){
                echo'<script>
                    alert("El usuari introduit no és correcte!!");
                    location.href="index.php";
                    </script>';
            }else if(isset($_POST['gpass']))
            {
                $valor=rand();
                $valor=hash('sha256',$valor);
    ...
    {
        $valid=0;
        $con=$db->prepare('SELECT mail, resetPassCode from users where mail=:email or username=:email');
        $con->bindParam(':email',$_GET['mail']);
        $con->execute();
        $valid=$con->fetch(PDO::FETCH_ASSOC);
        if($_GET['code']!=$valid['resetPassCode'] or $_GET['mail']!=$valid['mail'] or !isset($_GET['code']) or !isset($_GET['mail']))
